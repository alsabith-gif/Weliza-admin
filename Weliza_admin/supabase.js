...
// Used for per-client invoice auto-numbering — only pulls the number column.
async function getInvoicesByClient(clientId) {
  return sbGet('sales_invoices', `select=invoice_number&client_id=eq.${clientId}`, '');
}

// Used for global invoice auto-numbering — only pulls the number column,
// across every client (no client_id filter), so numbering never repeats
// between clients.
async function getAllInvoiceNumbers() {
  return sbGet('sales_invoices', `select=invoice_number`, '');
}

// Used for per-category invoice auto-numbering (e.g. Stitching's S00001 series)
// — only pulls the number column, across all clients.
async function getInvoicesByCategory(category) {
  return sbGet('sales_invoices', `select=invoice_number&category=eq.${category}`, '');
}
...
async function getDashboardStats() {
  ...
  const totalCogs      = invoices.reduce((s, i) => s + parseFloat(i.cost_total     || 0), 0);
  const grossPurchases = purchases.reduce((s, i) => s + parseFloat(i.grand_total   || 0), 0);
  const gstPaidPurchases = purchases.reduce((s, i) => s + parseFloat(i.gst_total   || 0), 0);
  const gstPaidExpenses  = expenses.reduce((s, e) => s + parseFloat(e.gst_amount   || 0), 0);
  const gstPaid        = gstPaidPurchases + gstPaidExpenses; // total input GST
...
async function getAllWorkers(search = '') {
  let params = 'select=*&order=name.asc';
  if (search) params += `&name=ilike.*${search}*`;
  return sbGet('workers', params);
}

async function createWorker(data) { return sbInsert('workers', data); }
async function updateWorker(id, data) { return sbUpdate('workers', id, data); }
async function deleteWorker(id) { return sbDelete('workers', id); }

// Export everything to window for use in admin.js
window.WelizaSupa = {
  uploadPdf,
  getSettings, saveSetting,
  getAllInvoices, getInvoiceById, createInvoice, updateInvoice, deleteInvoice, getInvoicesByClient, getInvoicesByCategory, getAllInvoiceNumbers,
