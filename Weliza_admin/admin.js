...
function setupInvoiceCreator() {
  ...
  document.getElementById('invBillCategory')?.addEventListener('change', async (e) => {
    const form = document.getElementById('invoiceForm');
    if (form && form.dataset.editId) return; // never renumber while editing an existing invoice
    const nextNum = await computeNextCategoryInvoiceNumber(e.target.value);
    if (nextNum) {
      const invInput = document.getElementById('invNumber');
      if (invInput) invInput.value = nextNum;
    }
  });
...
// Splits an invoice number into a leading prefix and a trailing number,
// e.g. "A0003" -> { prefix:"A", num:3, width:4 }. Returns null if it
// doesn't end in digits.
function parseInvoiceNumberTail(str) {
  const m = String(str || '').match(/^(.*?)(\d+)\s*$/);
  if (!m) return null;
  return { prefix: m[1], num: parseInt(m[2], 10), width: m[2].length };
}

// Looks at this client's own numbering prefix (from their Invoice Start),
// then checks ALL saved invoices — any client — that share that exact same
// prefix, and returns the next number after the highest one found. This
// keeps each client's own series distinct, while still preventing two
// different clients from colliding on the same number if they share a prefix.
// Falls back to the client's configured "invoice_start" when nothing matches yet.
async function computeNextInvoiceNumber(clientId, client) {
  try {
    const startTail = parseInvoiceNumberTail(client && client.invoice_start);
    const invoices = await SB().getAllInvoiceNumbers();
    let best = null;
    invoices.forEach(inv => {
      const p = parseInvoiceNumberTail(inv.invoice_number);
      if (!p) return;
      // Only count invoices that match this client's own prefix (when known).
      if (startTail && p.prefix !== startTail.prefix) return;
      if (!best || p.num > best.num) best = p;
    });
    if (best) {
      return best.prefix + String(best.num + 1).padStart(best.width, '0');
    }
  } catch (e) { console.error('computeNextInvoiceNumber error:', e); }
  return (client && client.invoice_start) ? client.invoice_start : null;
}

// Category-based auto-numbering (currently just "Stitching" → S00001, S00002, ...).
// Looks across ALL Stitching invoices (any client) and returns the next number
// in that series, independent of the per-client numbering above.
const CATEGORY_INVOICE_PREFIX = { Stitching: { prefix: 'S', width: 5 } };

async function computeNextCategoryInvoiceNumber(category) {
  const rule = CATEGORY_INVOICE_PREFIX[category];
  if (!rule) return null;
  try {
    const invoices = await SB().getInvoicesByCategory(category);
    let best = null;
    invoices.forEach(inv => {
      const p = parseInvoiceNumberTail(inv.invoice_number);
      if (p && (!best || p.num > best.num)) best = p;
    });
    const nextNum = best ? best.num + 1 : 1;
    return rule.prefix + String(nextNum).padStart(rule.width, '0');
  } catch (e) { console.error('computeNextCategoryInvoiceNumber error:', e); }
  return null;
}
  ...
  try {
    ...
    sel.onchange = function() {
      ...
      if (opt && opt.value) {
        ...
        // Auto-fill the next invoice number for this client — only for brand new
        // invoices, never while editing an existing one (that number must stay put).
        const form = document.getElementById('invoiceForm');
        if (form && !form.dataset.editId) {
          const client = allClientsCache.find(c => String(c.id) === opt.value);
          computeNextInvoiceNumber(opt.value, client).then(nextNum => {
            if (nextNum) {
              const invInput = document.getElementById('invNumber');
              if (invInput) invInput.value = nextNum;
            }
...
function buildInvoiceHTML(inv, biz, bank) {
  ...
  const WATERMARK_OPACITY = 0.07;  // 0 = invisible, 1 = fully solid. Try 0.05–0.15
  const WATERMARK_SIZE_PX = 6300;   // width of the watermark in px. Try 300–550
  ...
  return `<div style="position:relative;font-family:'Segoe UI',Arial,sans-serif;background:#fff;color:#222;font-size:12.5px;max-width:800px;margin:0 auto;padding:0 14px;box-sizing:border-box;">
    <link href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond:wght@600;700&display=swap" rel="stylesheet">

    <!-- Watermark: centered behind the whole invoice, sits below everything else -->
    <img src="weliza-watermark.png" alt="" style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:${WATERMARK_SIZE_PX}px;height:auto;opacity:${WATERMARK_OPACITY};pointer-events:none;z-index:5;">
    ...
    <div style="background:${G};color:#fff;padding:20px 26px;display:flex;justify-content:space-between;align-items:center;margin:0 -14px;">
      <div>
        <img src="weliza-logo-name.png" alt="${biz.name||'WELIZA'}" style="height:34px;width:auto;object-fit:contain;display:block;">
        <div style="font-size:10px;opacity:0.8;margin-top:8px;letter-spacing:0.3px;">This is an electronically generated document, no signature is required.</div>
      </div>
    ...
    <div style="display:flex;align-items:flex-start;gap:20px;padding-bottom:20px;">
      <div style="flex:1;margin-top:18px;">
        ${bank.acc_num ? `
        <div style="background:#f4fbf7;border:1.5px solid ${G};border-radius:7px;overflow:hidden;display:inline-block;min-width:210px;">
          <div style="background:${G};color:#fff;font-size:9px;font-weight:700;letter-spacing:1px;text-transform:uppercase;padding:6px 13px;">Bank Details</div>
          <table style="font-size:10.5px;border-collapse:collapse;width:100%;">
            <tr><td style="padding:5px 13px;color:#555;">Account Name</td><td style="font-weight:600;padding:5px 13px 5px 0;">${bank.acc_name||''}</td></tr>
            <tr style="background:#eef8f3;"><td style="padding:5px 13px;color:#555;">Account Number</td><td style="font-weight:600;padding:5px 13px 5px 0;">${bank.acc_num}</td></tr>
            <tr><td style="padding:5px 13px;color:#555;">IFSC</td><td style="font-weight:600;padding:5px 13px 5px 0;">${bank.ifsc||''}</td></tr>
            <tr style="background:#eef8f3;"><td style="padding:5px 13px;color:#555;">Account Type</td><td style="font-weight:600;padding:5px 13px 5px 0;">${bank.acc_type||''}</td></tr>
            <tr><td style="padding:5px 13px 8px;color:#555;">Bank</td><td style="font-weight:600;padding:5px 13px 8px 0;">${bank.bank_name||''}</td></tr>
          </table>
        </div>` : ''}
      </div>
...
async function saveInvoice(e) {
  ...
  // Editing an existing invoice needs one more confirmation before it actually saves.
  if (editId) {
    const ok = await confirmDialog('Are you sure you want to save these changes to the invoice?', 'Save Changes?');
    if (!ok) return;
  }
  ...
  try {
    if (editId) {
      await SB().updateInvoice(parseInt(editId), data);
      showToast('✓ Invoice updated!');
    } else {
      await SB().createInvoice(data);
      showToast('✓ Invoice saved!');
    }

    delete form.dataset.editId;
    const submitBtn = form.querySelector('button[type="submit"]');
    if (submitBtn) submitBtn.innerHTML = '<i data-lucide="save"></i> Save Invoice';
    document.getElementById('invoiceFormHeading').textContent = 'Invoice Details';
    document.getElementById('invoiceEditBanner').style.display = 'none';
    ...
    if (editId) goToTab('invoices-list');
  } catch(err) { showToast('Error: ' + err.message); }
}

// Opens the invoice in the New Invoice form, pre-filled, ready to edit.
// Shows a warning first — editing is only meant for corrections, not casual changes.
window.editInvoiceById = async (id) => {
  const ok = await confirmDialog('Are you sure you want to edit this invoice? Make sure this is really necessary.', 'Edit Invoice?');
  if (!ok) return;

  let inv = invoicesCache.find(i => i.id === id);
  if (!inv) { try { inv = await SB().getInvoiceById(id); } catch(e) { showToast('Could not load invoice'); return; } }
  if (!inv) { showToast('Invoice not found'); return; }

  goToTab('invoice-creator');

  const sel = document.getElementById('invClientSelect');
  const customInput = document.getElementById('invClientCustomName');
  if (inv.client_id) {
    // Saved client — show the dropdown, hide + un-require the custom name field
    if (sel) {
      sel.style.display = '';
      sel.required = true;
      sel.value = String(inv.client_id);
      sel.dataset.name    = inv.client_name    || '';
      sel.dataset.gstin   = inv.client_gstin   || '';
      sel.dataset.address = inv.client_address || '';
      sel.dataset.pan     = inv.client_pan     || '';
      sel.dataset.phone   = inv.client_phone   || '';
    }
    if (customInput) {
    ...
    // Custom / one-off client name — show the text field, hide + un-require the dropdown
    // (a hidden `required` field silently blocks the form from submitting, which was
    // the cause of edited invoices failing to save)
    if (sel) {
      sel.value = '';
      ['name','gstin','address','pan','phone'].forEach(k => delete sel.dataset[k]);
      sel.style.display = 'none';
    ...
    if (customInput) {
      ...
      customInput.value = inv.client_name || '';
    }
  }

  document.getElementById('invNumber').value       = inv.invoice_number || '';
  document.getElementById('invDate').value         = inv.date || '';
  document.getElementById('invBillCategory').value = inv.category || 'Retail';
  document.getElementById('invPaymentStatus').value= inv.status || 'Pending';

  const tbody = document.getElementById('invoiceItemsTbody');
  tbody.innerHTML = '';
  (inv.items || []).forEach(it => addInvoiceRow(it.desc, it.qty, it.rate, it.cost_price, it.gst_rate));
  if (!inv.items || !inv.items.length) addInvoiceRow();
  updateInvoiceTotals();

  const form = document.getElementById('invoiceForm');
  form.dataset.editId = id;
  const submitBtn = form.querySelector('button[type="submit"]');
  if (submitBtn) submitBtn.innerHTML = '<i data-lucide="save"></i> Save Changes';

  document.getElementById('invoiceFormHeading').textContent = 'Editing Invoice';
  document.getElementById('invoiceEditBannerText').textContent = `You are editing invoice ${inv.invoice_number || ''} — changes will overwrite the original.`;
  document.getElementById('invoiceEditBanner').style.display = 'flex';
  if (window.lucide) window.lucide.createIcons();
};

// Cancels edit mode and resets the New Invoice form back to a blank, ready-to-create state.
window.cancelInvoiceEdit = () => {
  const form = document.getElementById('invoiceForm');
  delete form.dataset.editId;
  const submitBtn = form.querySelector('button[type="submit"]');
  if (submitBtn) submitBtn.innerHTML = '<i data-lucide="save"></i> Save Invoice';
  ...
  ['name','gstin','address','pan','phone'].forEach(k => delete sel.dataset[k]);
  const customInput = document.getElementById('invClientCustomName');
  if (customInput && customInput.style.display !== 'none') toggleCustomClientInput();
  document.getElementById('invBillCategory').value = 'Retail';
...
async function loadInvoicesList() {
  ...
  try {
    ...
    invoices = invoices.slice().sort((a, b) => {
      ...
      return 0; // date case handled below via shared helper
    });
    if (sortBy === 'date') invoices = sortByDateDesc(invoices, 'date');
    ...
    tbody.innerHTML = invoices.map(inv => `
      <tr>
        ...
        <td>${inv.client_name || '—'}</td>
        <td>${inv.date}</td>
        <td><span class="badge badge-category">${inv.category || 'Retail'}</span></td>
        ...
        <td style="text-align:center;">
          <button class="btn btn-action" onclick="editInvoiceById(${inv.id})" title="Edit"><i data-lucide="pencil"></i></button>
          ${inv.pdf_url
...
function setupPurchaseLog() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (editId) {
      const ok = await confirmDialog('Are you sure you want to save these changes to the purchase?', 'Save Changes?');
      if (!ok) return;
    }
    ...
    try {
      ...
      if (editId) {
        await SB().updatePurchase(parseInt(editId), data);
      ...
      delete form.dataset.editId;
      const submitBtn = form.querySelector('button[type="submit"]');
      if (submitBtn) submitBtn.innerHTML = '<i data-lucide="plus-circle"></i> Log Purchase';
      document.getElementById('purchaseFormHeading').textContent = 'Log Purchase';
      document.getElementById('purchaseEditBanner').style.display = 'none';
      form.reset();
...
// Opens the purchase in the Log Purchase form, pre-filled, ready to edit.
window.editPurchaseById = async (id) => {
  const ok = await confirmDialog('Are you sure you want to edit this purchase? Make sure this is really necessary.', 'Edit Purchase?');
  if (!ok) return;

  let p = purchasesCache.find(x => x.id === id);
  if (!p) { try { p = await SB().getPurchaseById(id); } catch(e) { showToast('Could not load purchase'); return; } }
  if (!p) { showToast('Purchase not found'); return; }

  document.querySelector('.nav-link[data-tab="purchases"]')?.click();

  document.getElementById('purSupplier').value       = p.dealer_name || '';
  document.getElementById('purSupplierGstin').value  = p.dealer_gstin || '';
  document.getElementById('purBillNumber').value     = p.invoice_number || '';
  document.getElementById('purDate').value           = p.date || '';
  const taxable = parseFloat(p.taxable_total || 0);
  const gstAmt  = parseFloat(p.gst_total || 0);
  const rate    = taxable > 0 ? Math.round((gstAmt / taxable) * 100) : 0;
  const rateOptions = ['0','5','12','18'];
  document.getElementById('purGstRate').value = rateOptions.includes(String(rate)) ? String(rate) : '5';
  document.getElementById('purTaxable').value    = taxable.toFixed(2);
  document.getElementById('purGstAmount').value  = gstAmt.toFixed(2);
  document.getElementById('purTotal').value      = parseFloat(p.grand_total || 0).toFixed(2);
  ...
  document.getElementById('purchaseFormHeading').textContent = 'Editing Purchase';
  document.getElementById('purchaseEditBannerText').textContent = `You are editing the purchase from ${p.dealer_name || 'this supplier'} — changes will overwrite the original.`;
  document.getElementById('purchaseEditBanner').style.display = 'flex';
  if (window.lucide) window.lucide.createIcons();
};

// Cancels edit mode and resets the Log Purchase form back to a blank, ready-to-create state.
...
document.getElementById('cancelPurchaseEditBtn')?.addEventListener('click', cancelPurchaseEdit);
...
async function loadPurchasesList() {
  ...
  try {
    let purchases = await SB().getAllPurchases();
    purchases = sortByDateDesc(purchases, 'date');
    ...
    tbody.innerHTML = purchases.map(p => `
      <tr>
        ...
        <td style="text-align:center;">
          <button class="btn btn-action" onclick="editPurchaseById(${p.id})" title="Edit"><i data-lucide="pencil"></i></button>
...
function setupClientCRM() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    const data = {
      ...
      invoice_start: document.getElementById('cliInvoiceStart')?.value?.trim() || null,
    ...
    try {
      ...
      document.getElementById('clientEditBanner').style.display = 'none';
      loadClientsList();
      populateClientDropdown();
    } catch(err) { showToast('Error: ' + err.message); }
  });
...
window.editClient = (id) => {
  ...
  document.getElementById('cliInvoiceStart').value = c.invoice_start || '';
  ...
  document.getElementById('crmFormHeading').textContent = 'Editing Client';
  document.getElementById('clientEditBannerText').textContent = `You are editing "${c.name || 'this client'}".`;
  document.getElementById('clientEditBanner').style.display = 'flex';
  if (window.lucide) window.lucide.createIcons();
...
function setupDealers() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      form.reset();
      const submitBtn = document.getElementById('btnDealerSubmit');
      if (submitBtn) submitBtn.innerHTML = '<i data-lucide="plus"></i><span>Add Dealer</span>';
      document.getElementById('dealerFormHeading').textContent = 'Add / Edit Dealer';
      document.getElementById('dealerEditBanner').style.display = 'none';
      if (window.lucide) window.lucide.createIcons();
      loadDealersList();
    } catch(err) { showToast('Error: ' + err.message); }
  });
  document.getElementById('btnCancelDealerEdit')?.addEventListener('click', () => {
    delete form.dataset.editId;
    form.reset();
    const submitBtn = document.getElementById('btnDealerSubmit');
    if (submitBtn) submitBtn.innerHTML = '<i data-lucide="plus"></i><span>Add Dealer</span>';
...
window.editDealer = (id) => {
  ...
  const submitBtn = document.getElementById('btnDealerSubmit');
  if (submitBtn) submitBtn.innerHTML = '<i data-lucide="save"></i><span>Save Changes</span>';
  document.getElementById('dealerFormHeading').textContent = 'Editing Dealer';
  document.getElementById('dealerEditBannerText').textContent = `You are editing "${d.name || 'this dealer'}".`;
  document.getElementById('dealerEditBanner').style.display = 'flex';
  if (window.lucide) window.lucide.createIcons();
...
function setupExpenses() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (editId) {
      const ok = await confirmDialog('Are you sure you want to save these changes to the expense?', 'Save Changes?');
      if (!ok) return;
    }
    ...
    try {
      ...
      if (editId) {
        await SB().updateExpense(parseInt(editId), data);
      ...
      delete form.dataset.editId;
      const submitBtn = form.querySelector('button[type="submit"]');
      if (submitBtn) submitBtn.innerHTML = '<i data-lucide="plus"></i><span>Add Expense</span>';
      document.getElementById('expenseFormHeading').textContent = 'Add Expense';
...
// Opens the expense in the Add Expense form, pre-filled, ready to edit.
window.editExpenseById = async (id) => {
  const ok = await confirmDialog('Are you sure you want to edit this expense? Make sure this is really necessary.', 'Edit Expense?');
  if (!ok) return;

  const exp = expensesCache.find(x => x.id === id);
  if (!exp) { showToast('Expense not found'); return; }

  document.querySelector('.nav-link[data-tab="expenses"]')?.click();

  document.getElementById('expCategory').value    = exp.category || '';
  document.getElementById('expAmount').value      = exp.amount || '';
  document.getElementById('expGstRate').value     = exp.gst_rate || 0;
  document.getElementById('expGstAmount').value   = exp.gst_amount ? parseFloat(exp.gst_amount).toFixed(2) : '';
  document.getElementById('expTotalAmount').value = exp.amount ? (parseFloat(exp.amount)+parseFloat(exp.gst_amount||0)).toFixed(2) : '';
  document.getElementById('expDescription').value = exp.description || '';
  document.getElementById('expDate').value        = exp.date || '';
  document.getElementById('expPaidTo').value      = exp.paid_to || '';
  document.getElementById('expPaymentMode').value = exp.payment_mode || 'Cash';
  document.getElementById('expNotes').value       = exp.notes || '';

  const workerRow = document.getElementById('workerSelectRow');
  if (exp.category === 'Salary') {
    workerRow.style.display = '';
    await populateWorkerDropdown();
    if (exp.worker_id) document.getElementById('expWorkerSelect').value = String(exp.worker_id);
  } else {
  ...
  document.getElementById('expenseFormHeading').textContent = 'Editing Expense';
  document.getElementById('expenseEditBannerText').textContent = `You are editing this expense (${exp.category || ''}) — changes will overwrite the original.`;
  document.getElementById('expenseEditBanner').style.display = 'flex';
  if (window.lucide) window.lucide.createIcons();
};

// Cancels edit mode and resets the Add Expense form back to a blank, ready-to-create state.
...
async function loadExpensesList() {
  ...
  try {
    let expenses = await SB().getAllExpenses({ category });
    expenses = sortByDateDesc(expenses, 'date');
    ...
    tbody.innerHTML = expenses.map(exp => `
      <tr>
        ...
        <td style="text-align:center;">
          <button class="btn btn-action" onclick="editExpenseById(${exp.id})" title="Edit"><i data-lucide="pencil"></i></button>
...
function setupInventory() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      const submitBtn = form.querySelector('button[type="submit"]');
      if (submitBtn) submitBtn.innerHTML = '<i data-lucide="plus"></i><span>Add Item</span>';
      document.getElementById('inventoryFormHeading').textContent = 'Add / Edit Item';
      document.getElementById('inventoryEditBanner').style.display = 'none';
      if (window.lucide) window.lucide.createIcons();
      loadInventoryList();
    } catch(err) { showToast('Error: ' + err.message); }
  });
...
// Opens the item in the Inventory form, pre-filled, ready to edit.
// Inventory is freely editable — no confirmation warnings, unlike Invoices/Purchases/Expenses.
window.editInventoryById = (id) => {
  const item = inventoryListCache.find(x => x.id === id);
  ...
  document.getElementById('invSellPrice').value = item.sell_price ?? '';
  document.getElementById('invLowStock').value  = item.low_stock_alert ?? 5;
  ...
  document.getElementById('inventoryFormHeading').textContent = 'Editing Item';
  document.getElementById('inventoryEditBannerText').textContent = `You are editing "${item.name || 'this item'}".`;
  document.getElementById('inventoryEditBanner').style.display = 'flex';
  if (window.lucide) window.lucide.createIcons();
  form.scrollIntoView({ behavior: 'smooth', block: 'start' });
};

// Cancels edit mode and resets the Add / Edit Item form back to a blank, ready-to-add state.
...
document.getElementById('cancelInventoryEditBtn')?.addEventListener('click', cancelInventoryEdit);

let inventoryListCache = [];
  ...
  try {
    ...
    tbody.innerHTML = items.map(item => {
      ...
      return `<tr ${low?'style="background:#fff5f5;"':''}>
        ...
        <td style="text-align:center;">
          <button class="btn btn-action" onclick="editInventoryById(${item.id})" title="Edit"><i data-lucide="pencil"></i></button>
...
async function loadReports() {
  ...
  try {
    ...
    reportDataCache = {
      sortedMonthsAsc, sortedMonthsDesc, months, profitByMonth,
      bestMonth, bestProfit, insights,
      stats, margin, gstNet,
      totalSalesAll, totalPurchasesAll, totalExpensesAll,
      salesPct, purchasesPct, expensesPct,
      expenseByCategory, expenseCatLabels, expenseCatValues, totalExpenseAmt
    };
...
// ─── CLEAN REPORT PDF (separate from the on-screen dashboard) ──
// Builds a minimal, print-friendly, single-page A4 report using Helvetica,
// with a large low-opacity Weliza watermark — independent of the dark
// dashboard theme so it prints cleanly.
function buildReportPrintHTML(d) {
  const G = '#1F5138';
  const fmt = (v) => formatCurrency(v);
  const recentMonths = d.sortedMonthsDesc.slice(0, 6); // keep the printed table to one page

  const monthRowsHtml = recentMonths.map(m => {
    const dd = d.months[m];
    const profit = d.profitByMonth[m];
    return `<tr>
      <td>${m}</td>
      <td>${fmt(dd.sales)}</td>
      <td>${fmt(dd.purchases)}</td>
      <td>${fmt(dd.expenses)}</td>
      <td style="color:${profit>=0?G:'#c0392b'};font-weight:700;">${fmt(profit)}</td>
    </tr>`;
  }).join('') || `<tr><td colspan="5" style="text-align:center;color:#aaa;padding:16px;">No data yet</td></tr>`;

  const insightsHtml = d.insights.length
    ? d.insights.map(i => `<div style="display:flex;gap:8px;font-size:13px;line-height:1.5;">
        <span style="width:5px;height:5px;border-radius:50%;background:#C09A59;margin-top:7px;flex-shrink:0;"></span>
        <span>${i.text}</span>
      </div>`).join('')
    : `<div style="font-size:13px;color:#888;">Not enough data yet for insights.</div>`;

  const expenseColors = ['#1F5138','#C09A59','#7C9E8E','#B4B2A9','#8A9B6E','#4E9DE8'];
  const legendHtml = d.expenseCatLabels.map((c,i) => `
    <div style="display:flex;justify-content:space-between;gap:16px;font-size:12.5px;">
      <span><span style="display:inline-block;width:8px;height:8px;border-radius:2px;background:${expenseColors[i%expenseColors.length]};margin-right:6px;"></span>${c}</span>
      <span>${((d.expenseByCategory[c]/d.totalExpenseAmt)*100).toFixed(0)}%</span>
    </div>`).join('');

  return `<div class="page">
    <img class="watermark" src="weliza-icon-mark-dark.png">
    <div class="content">
      <div class="rep-header">
        <img class="wordmark" src="weliza-logo-name-dark.png" alt="Weliza">
        <div class="period">
          <div class="t">Business Report</div>
          <div class="d">${d.sortedMonthsAsc[0]||''} &ndash; ${d.sortedMonthsDesc[0]||''}</div>
        </div>
      </div>
      ...
      <div class="kpi-row">
        <div class="kpi">
          ...
          <div class="value">${fmt(d.stats.netProfit)}</div>
          <div class="sub">${d.margin.toFixed(0)}% margin</div>
        </div>
        <div class="kpi ${d.gstNet>0?'warn':''}">
          <div class="label">GST ${d.gstNet>0?'Payable':'Position'}</div>
          <div class="value">${fmt(Math.abs(d.gstNet))}</div>
          <div class="sub">${d.gstNet>0?'due this cycle':'balanced'}</div>
        </div>
        <div class="kpi">
          <div class="label">Best Month</div>
          <div class="value" style="font-size:1.2rem;">${d.bestMonth?fmt(d.bestProfit):'—'}</div>
          <div class="sub">${d.bestMonth||''}</div>
        </div>
        <div class="kpi">
          <div class="label">Outstanding</div>
          <div class="value" style="font-size:1.2rem;">${fmt(d.stats.outstanding)}</div>
      ...
      <div class="sec-title">Money In vs Money Out</div>
      <div class="money-bar">
        <div style="width:${d.salesPct}%;background:${G};"></div>
        <div style="width:${d.purchasesPct}%;background:#C09A59;"></div>
        <div style="width:${d.expensesPct}%;background:#B4B2A9;"></div>
      </div>
      <div class="money-legend">
        <span><span class="sw" style="background:${G};"></span>Sales ${fmt(d.totalSalesAll)}</span>
        <span><span class="sw" style="background:#C09A59;"></span>Purchases ${fmt(d.totalPurchasesAll)}</span>
        <span><span class="sw" style="background:#B4B2A9;"></span>Expenses ${fmt(d.totalExpensesAll)}</span>
      </div>

      <div class="sec-title">Net Profit Trend</div>
      <div class="chart-wrap"><canvas id="pdfTrendChart" role="img" aria-label="Line chart of net profit by month"></canvas></div>

      <div class="sec-title">Where Expenses Go</div>
      <div class="two-col">
        <div class="chart-wrap donut">${d.expenseCatLabels.length ? '<canvas id="pdfExpenseChart" role="img" aria-label="Donut chart of expense categories"></canvas>' : ''}</div>
        <div class="legend-list">${legendHtml || '<div style="font-size:12.5px;color:#888;">No expenses logged yet</div>'}</div>
      </div>

      <div class="sec-title">Monthly Breakdown</div>
      <table class="rep-tbl">
        <thead><tr><th>Month</th><th>Sales</th><th>Purchases</th><th>Expenses</th><th>Profit</th></tr></thead>
        <tbody>${monthRowsHtml}</tbody>
      </table>

      <div class="footer">Generated ${new Date().toLocaleDateString('en-IN', { day:'numeric', month:'long', year:'numeric' })} &mdash; Weliza Design, Kozhikode</div>
    </div>
  </div>`;
}

function printCleanReport() {
  if (!reportDataCache) { showToast('Load the reports page first'); return; }
  const d = reportDataCache;
  const trendLabels = d.sortedMonthsAsc;
  const trendData = trendLabels.map(m => d.profitByMonth[m]);

  const w = window.open('', '_blank');
  w.document.write(`<!DOCTYPE html><html><head><title>Weliza Business Report</title>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/Chart.js/4.4.1/chart.umd.js"><\/script>
    <style>
      * { box-sizing: border-box; }
      body { margin:0; padding:20px; background:#ececec; font-family:'Helvetica Neue',Helvetica,Arial,sans-serif; color:#222; }
      .page { position:relative; width:210mm; min-height:297mm; margin:0 auto; background:#fff; padding:34px 46px 26px; overflow:hidden; }
      .watermark { position:absolute; top:50%; left:50%; transform:translate(-50%,-50%); width:520px; opacity:0.07; z-index:0; pointer-events:none; }
      .content { position:relative; z-index:1; }
      .rep-header { display:flex; justify-content:space-between; align-items:flex-end; padding-bottom:14px; border-bottom:2px solid #1F5138; }
      .rep-header img.wordmark { height:28px; width:auto; object-fit:contain; display:block; }
      .rep-header .period { text-align:right; }
      .rep-header .period .t { font-size:11px; text-transform:uppercase; letter-spacing:1px; color:#888; }
      .rep-header .period .d { font-size:13px; font-weight:700; color:#222; margin-top:3px; }
      .kpi-row { display:flex; gap:16px; margin:24px 0; }
      .kpi { flex:1; border:1px solid #e4e4e0; border-left:3px solid #1F5138; border-radius:4px; padding:12px 15px; }
      .kpi.warn { border-left-color:#C09A59; }
      .kpi .label { font-size:10px; text-transform:uppercase; letter-spacing:0.6px; color:#888; }
      .kpi .value { font-size:1.4rem; font-weight:700; margin-top:4px; color:#1a1a1a; }
      .kpi .sub { font-size:10.5px; color:#999; margin-top:2px; }
      .sec-title { font-size:12.5px; font-weight:700; text-transform:uppercase; letter-spacing:1px; color:#1F5138; margin:22px 0 10px; padding-bottom:5px; border-bottom:1px solid #e4e4e0; }
      .insights { display:flex; flex-direction:column; gap:7px; }
      .money-bar { display:flex; height:9px; border-radius:5px; overflow:hidden; }
      .money-legend { display:flex; gap:18px; margin-top:9px; font-size:11.5px; color:#666; flex-wrap:wrap; }
      .money-legend span.sw { display:inline-block; width:8px; height:8px; border-radius:2px; margin-right:5px; }
      .chart-wrap { position:relative; height:150px; margin-top:4px; }
      .two-col { display:flex; gap:28px; align-items:center; }
      .two-col .chart-wrap.donut { flex-shrink:0; width:120px; height:120px; margin:0; }
      .legend-list { display:flex; flex-direction:column; gap:6px; font-size:12px; }
      table.rep-tbl { width:100%; border-collapse:collapse; font-size:12px; margin-top:4px; }
      table.rep-tbl th { text-align:left; font-size:10px; text-transform:uppercase; letter-spacing:0.5px; color:#888; padding:6px 8px; border-bottom:1px solid #1F5138; }
      table.rep-tbl td { padding:7px 8px; border-bottom:1px solid #eee; }
      table.rep-tbl tr:nth-child(even) td { background:#fafbfa; }
      .footer { text-align:center; font-size:10.5px; color:#aaa; margin-top:26px; padding-top:12px; border-top:1px solid #eee; }
      @media print {
        @page { size: A4 portrait; margin: 0; }
    ...
    </head><body>${buildReportPrintHTML(d)}
    <script>
      window.onload = () => {
        const trendCtx = document.getElementById('pdfTrendChart');
        if (trendCtx) {
          new Chart(trendCtx, {
            type: 'line',
            data: { labels: ${JSON.stringify(trendLabels)}, datasets: [{
              data: ${JSON.stringify(trendData)}, borderColor: '#1F5138',
              backgroundColor: 'rgba(31,81,56,0.08)', borderWidth: 2,
              pointRadius: 3, pointBackgroundColor: '#1F5138', fill: true, tension: 0.3
            }]},
            options: { responsive:true, maintainAspectRatio:false, plugins:{legend:{display:false}},
              scales:{ y:{ grid:{color:'#eee'}, ticks:{font:{size:10},color:'#999'} }, x:{ grid:{display:false}, ticks:{font:{size:10},color:'#999'} } } }
          });
        }
        const expCtx = document.getElementById('pdfExpenseChart');
        if (expCtx) {
          new Chart(expCtx, {
            type: 'doughnut',
            data: { labels: ${JSON.stringify(d.expenseCatLabels)}, datasets: [{
              data: ${JSON.stringify(d.expenseCatValues)},
              backgroundColor: ${JSON.stringify(['#1F5138','#C09A59','#7C9E8E','#B4B2A9','#8A9B6E','#4E9DE8'])},
...
function setupWorkers() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      document.getElementById('workerEditBanner').style.display = 'none';
      loadWorkersList();
...
async function loadWorkersList() {
  ...
  try {
    ...
    workers.sort((a, b) => {
      if (a.join_date && b.join_date) return new Date(b.join_date) - new Date(a.join_date);
      if (a.created_at && b.created_at) return new Date(b.created_at) - new Date(a.created_at);
      return (b.id||0) - (a.id||0);
    });
...
window.editWorker = async (id) => {
  ...
  document.getElementById('workerFormHeading').textContent = 'Editing Worker';
  document.getElementById('workerEditBannerText').textContent = `You are editing "${w.name || 'this worker'}".`;
  document.getElementById('workerEditBanner').style.display = 'flex';
  if (window.lucide) window.lucide.createIcons();
...
// ─── REFRESH BUTTON ─────────────────────────────────────────────
// iPhones running this as a "Add to Home Screen" app lose Safari's pull-to-refresh
// and reload button, since standalone mode hides all browser chrome. This button
// gives an equivalent way to reload fresh data at any time.
...
// Reusable confirm dialog (used for Edit/Save warnings on Invoices, Purchases, Expenses).
// Returns a Promise<boolean> — true if the user clicked Continue, false otherwise.
function confirmDialog(message, title = 'Are you sure?') {
  return new Promise(resolve => {
    ...
    if (!modal) { resolve(true); return; } // fail-safe: never block the user if the modal is missing
...
// Sorts a list of records by a date field, newest first / oldest last.
function sortByDateDesc(list, dateField = 'date') {
  return list.slice().sort((a, b) => new Date(b[dateField]) - new Date(a[dateField]));
}
