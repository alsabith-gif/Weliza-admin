...
// Looks at this client's actual saved invoices and returns the next number
// in their series (same prefix/padding as their highest existing invoice).
// Falls back to the client's configured "invoice_start" if they have none yet.
async function computeNextInvoiceNumber(clientId, client) {
  try {
    const invoices = await SB().getInvoicesByClient(clientId);
    let best = null;
    invoices.forEach(inv => {
